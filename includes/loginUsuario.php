...
if(!empty($_POST)){
    if(empty($_POST['login']) || empty($_POST['pass'])){
        ...
        $sql = 'SELECT * FROM usuarios u JOIN rol r ON u.rol = r.rol_id WHERE u.usuario = ? AND u.estado != 0';
        ...
        if($query->rowCount() > 0){
            if(password_verify($pass, $result['clave'])){
                if($result['estado']==1){
                    ...
                    echo '<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert">&times;</button>Usuario actualmente inactivo. Comunicarse con soporte</div>';
                }
                
            }else{
                echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Usuario y/o contraseña incorrectos</div>';
            }
        } else {
            echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Usuario y/o contraseña incorrectos</div>';
        }
